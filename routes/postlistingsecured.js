...
const nametext = new TextRun(letter_data.custname);
  ...
  const headertext = new TextRun("RE: DEMAND FOR OUTSTANDING BALANCES DUE ON (TYPE OF FACILITY) FACILITIES BASE NO. " + letter_data.acc + " AND NOTICE OF LISTING AT THE CREDIT REFERENCE BUREAU ISSUED PURSUANT TO REGULATION 50 (1) (b) OF THE CREDIT REFERENCE BUREAU REGULATIONS, 2013");
  ...
  document.createParagraph(" ");
  document.createParagraph("We wish to notify you that you have breached terms of the letter of offer dated {{offerdate}}. by defaulting in repaying your monthly repayments and as such your account is in arrears of " + letter_data.accounts[0].currency + ' ' + numeral(Math.abs(letter_data.accounts[0].oustbalance)).format('0,0.00') + " .dr as at " + DATE + " which continues to accrue interest at xxx% per annum (equivalent to Kenya Bank's Reference Rate (KBRR) currently at xxxx% plus a margin of xxx% (K)) and penal charges of 0.5% per month. Further, you owe the Bank the total sum of " + letter_data.accounts[0].currency + ' ' + numeral(Math.abs(letter_data.accounts[0].oustbalance)).format('0,0.00') + " dr as at " + DATE + " being the outstanding amount on the facility, which continues to accrue interest at xxx% per annum (equivalent to Kenya Bank's Reference Rate (KBRR) currently at xxxx% plus a margin of xxx% (K)) until full payment, full particulars whereof is well within your knowledge. ");
  document.createParagraph(" ");
